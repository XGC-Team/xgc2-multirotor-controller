...
dpkg -s "ros-${ROS_DISTRO}-xgc2-estimator-hover-thrust-msgs" >/dev/null
dpkg -s "ros-${ROS_DISTRO}-xgc2-estimator-rigid-state-msgs" >/dev/null
dpkg -s "ros-${ROS_DISTRO}-xgc2-multirotor-reference-trajectory-msgs" >/dev/null
dpkg -s "ros-${ROS_DISTRO}-xgc2-px4-multirotor-controller-msgs" >/dev/null
...
rosmsg show multirotor_reference_trajectory_msgs/AnalyticReference | grep -q '^uint16 analytic_type$'
